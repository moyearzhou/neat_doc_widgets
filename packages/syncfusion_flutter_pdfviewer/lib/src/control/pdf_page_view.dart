...
class PdfPageViewState extends State<PdfPageView> {
  ...
  Future<void> _getImage(Size viewportSize, double zoomLevel) async {
    ...
    if (widget.pdfDocument != null && _previousImageFactor != imageFactor) {
      ...
      try {
        final Stopwatch channelStopwatch = Stopwatch()..start();
        ...
        await _pageImageOperation?.value.then((Uint8List? pageImage) {
          final int channelMs = channelStopwatch.elapsedMilliseconds;
          if (pageImage != null) {
            ...
            _createImage(pageImage, _imageWidth, _imageHeight).then((
              ...
              final int renderedPage = widget.pageIndex + 1;
              final Stopwatch? frameStopwatch =
              ...
              if (frameStopwatch != null) {
                debugPrint(
                  '[Syncfusion PDF][dartPage] page=$renderedPage '
                  'size=${_imageWidth}x$_imageHeight '
                  'bytes=${pageImage.length} channelMs=$channelMs '
                  'decodeMs=$decodeMs',
                );
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  debugPrint(
                    '[Syncfusion PDF][dartPageFrame] page=$renderedPage '
                    'postFrameMs=${frameStopwatch.elapsedMilliseconds}',
                  );
                });
              }
            });
          ...
          await imageFuture.then((Uint8List? tileImage) {
            if (tileImage != null &&
              ...
              _createImage(
                ...
                if (frameStopwatch != null) {
                  debugPrint(
                    '[Syncfusion PDF][dartTile] page=$renderedPage '
                    'size=${tileImageSize.width.toInt()}x${tileImageSize.height.toInt()} '
                    'bytes=${tileImage.length} channelMs=$channelMs '
                    'decodeMs=$decodeMs',
                  );
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    debugPrint(
                      '[Syncfusion PDF][dartTileFrame] page=$renderedPage '
                      'postFrameMs=${frameStopwatch.elapsedMilliseconds}',
                    );
                  });
                }
