...
class SfPdfViewerState extends State<SfPdfViewer> with WidgetsBindingObserver {
        ...
        if (field.items != null && field.items!.count > 0) {
          ...
          for (int j = 0; j < field.items!.count; j++) {
            ...
            if (item is PdfCheckBoxItem) {
              ...
              final PdfCheckboxFormField groupedFormField = groupedItemHelper
                  .getFormField();
    ...
    for (int pageIndex = 0; pageIndex < _document!.pages.count; pageIndex++) {
      ...
      for (
        ...
        if (pdfAnnotation is PdfTextMarkupAnnotation &&
            (pdfAnnotation.textMarkupAnnotationType ==
  ...
  Future<void> _performTextExtraction() async {
    ...
    // Full-document extraction duplicates the parsed PDF in an isolate and can
    // exceed the iOS memory limit for large files. Defer it until search is used.
  ...
  Widget _getReplacementWidget(int pageNumber, Size size) {
    final double pageSpacing = pageNumber == _pdfViewerController.pageCount
        ? 0.0
        : widget.pageSpacing;
  ...
  void _checkVisiblePages() {
    ...
    if (widget.pageLayoutMode == PdfPageLayoutMode.single) {
      ...
      final int cacheStart =
          (_pdfViewerController.pageNumber - _pageImageCacheRadius).clamp(
            1,
            _pdfViewerController.pageCount,
          );
      final int cacheEnd =
      ...
      // Keep a bounded page-image window so normal and reverse scrolling can
      // reuse rendered pages without retaining the entire document in memory.
      for (
        int pageNumber = 1;
    ...
    if (property == 'jumpToBookmark') {
      ...
      if (kIsWeb) {
        ...
        if (_isTextExtractionCompleted) {
          final String searchText = _pdfViewerController._searchText
              .toLowerCase();
  ...
  void _getPageSizes() {
    ...
    for (int pageIndex = 0; pageIndex < _document!.pages.count; pageIndex++) {
      ...
      final double width = isRotatedTo90or270
          ? page.size.height
          : page.size.width;
      final double height = isRotatedTo90or270
          ? page.size.width
          : page.size.height;
